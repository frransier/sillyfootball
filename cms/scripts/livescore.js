...
async function Livescore(m) {
  ...
  const events = match.events.filter((x) => x.type === "Goal");
  const sanityEvents = events.map((event, index) => ({
    _type: "event",
  ...
  client
    ...
    .commit();
  const goals = events.map((event) => `${event.player_id}-${event.team_id}`);
